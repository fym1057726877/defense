...
from ours.diffusion.models import MemoryUnet, MultilevelMemoryUnet
...
class DirectTrainActorReconstruct:
    def __init__(
        ...
        #     get_project_path(project_name="Defense"), "pretrained", "Multi_Memory_Unet_for_DM.pth")
        # self.unet.load_state_dict(torch.load(self.save_path))
    ...
    def defense(self, img):
        first_denoise = iter_denoise(self.unet, imgs=img, t=50, sampler="ddim", progress=False)
        second_denoise, mem_loss = self.autoencoder(first_denoise)
        return second_denoise
