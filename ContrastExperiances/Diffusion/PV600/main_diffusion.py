...
class TrainDiffusion:
    def __init__(
        ...
        self.save_path = os.path.join(get_project_path("Defense"), "ContrastExperiances", "Diffusion", "PV600", "diffusion.pth")
    ...
    def test_defense_one(self):
        ...
        log = simple_test_defense(defense_model=self.unet, attack_type="SPSA", eps=0.2,
    ...
    def total_test(self):
        torch.manual_seed(10)
        logging.basicConfig(filename="./logs/whitebox.logs", filemode="w", level=logging.INFO, format="")
        ...
        for classifier_name in classifier_name_list:
            log1 = simple_test_defense(defense_model=self.unet, defense_fn=defense, attack_type="FGSM", eps=0.03,
                                       classifier_name=classifier_name, device=self.device)
            print(log1)
            logging.info(log1)
            log2 = simple_test_defense(defense_model=self.unet, defense_fn=defense, attack_type="FGSM", eps=0.05,
                                       classifier_name=classifier_name, device=self.device)
            print(log2)
            logging.info(log2)
            log3 = simple_test_defense(defense_model=self.unet, defense_fn=defense, attack_type="PGD", eps=0.03,
                                       classifier_name=classifier_name, device=self.device)
            print(log3)
            logging.info(log3)
            log4 = simple_test_defense(defense_model=self.unet, defense_fn=defense, attack_type="PGD", eps=0.05,
                                       classifier_name=classifier_name, device=self.device)
            print(log4)
            logging.info(log4)
    ...
    def BlackBoxAttackTest(self):
        ...
        for classifier_name in classifier_name_list:
            log1 = simple_test_defense(defense_model=self.unet, attack_type="SPSA", eps=0.1,
                                       testloader=self.testloader, progress=True, defense_fn=defense,
                                       classifier_name=classifier_name, device=self.device)
            print(log1)
            logging.info(log1)
