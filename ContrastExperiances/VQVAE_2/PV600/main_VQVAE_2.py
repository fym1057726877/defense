...
class Trainer:
    def __init__(
        ...
        self.save_path = os.path.join(
            get_project_path(project_name="Defense"), "ContrastExperiances", "VQVAE_2", "PV600", "VQVAE_2_1.pth")
        self.model.load_state_dict(torch.load(self.save_path))

        self.trainloader = trainloader
        self.testloader = testloader

        self.classifier_name = "FV_CNN_PV600"

    def train(self, epochs, progress=True):
        start = time()
        loss_fn = nn.MSELoss()
        best_acc = 0
        logging.basicConfig(filename="logs/vqvae_v2_train.log", filemode="w", level=logging.INFO, format="")
    ...
    def total_test(self):
        ...
        logging.basicConfig(filename="./logs/vqvae_v2_test.log", filemode="w", level=logging.INFO, format="")
