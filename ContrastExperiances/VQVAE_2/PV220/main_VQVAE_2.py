...
class Trainer:
    def __init__(
        ...
        self.save_path = os.path.join(
            get_project_path(project_name="Defense"), "ContrastExperiances", "VQVAE_2", "PV220", "VQVAE_v2_pv220.pth")
    ...
    def train(self, epochs, progress=True):
        ...
        loss_fn = nn.MSELoss()
        best_acc = 0
        logging.basicConfig(filename="logs/vqvae_v2_train_pv220.log", filemode="w", level=logging.INFO, format="")
        ...
        for epoch in range(epochs):
            ...
            for step, (img, _) in iterobject:
                ...
                rec, vq_loss = self.model(img)
                loss = vq_loss + loss_fn(rec, img)
                loss.backward()
                optimizer.step()
                epoch_loss += loss
    ...
    def total_test(self):
        ...
        logging.basicConfig(filename="./logs/vqvae_v2_test_pv220.log", filemode="w", level=logging.INFO, format="")
