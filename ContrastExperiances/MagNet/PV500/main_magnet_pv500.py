...
from classifier.models import load_ModelB_FV500
...
class Trainer:
    def __init__(
        ...
        self.save_path = os.path.join(
            get_project_path("Defense"), "ContrastExperiances", "MagNet", "PV500", "magnet_pv500.pth")
    ...
    def eval_classifier(self):
        target_classifier = load_ModelB_FV500()
    ...
    def total_test1(self):
        ...
        for classifier_name in classifier_name_list:
            log1 = simple_test_defense(defense_model=self.model, attack_type="FGSM", eps=0.2,
                                       testloader=self.testloader, progress=False,
                                       classifier_name=classifier_name, device=self.device)
            print(log1)
            log2 = simple_test_defense(defense_model=self.model, attack_type="PGD", eps=0.2,
                                       testloader=self.testloader, progress=False,
                                       classifier_name=classifier_name, device=self.device)
            print(log2)
    ...
    def total_test(self):
        ...
        logging.basicConfig(filename="./logs/magnet_test_white.logs", filemode="w", level=logging.INFO, format="")
        classifier_name_list = [
            "FV_CNN_FV500",
            "PV_CNN_FV500",
        ...
        for classifier_name in classifier_name_list:
            ...
            print(log1)
            logging.info(log1)
            log2 = simple_test_defense(defense_model=self.model, attack_type="PGD", eps=0.3,
                                       testloader=self.testloader, progress=False,
                                       classifier_name=classifier_name, device=self.device)
            print(log2)
            logging.info(log2)
    ...
    def BlackBoxAttackTest(self):
        torch.manual_seed(10)
        logging.basicConfig(filename="./logs/magnet_test_black.logs", filemode="w", level=logging.INFO, format="")
        classifier_name_list = [
            "FV_CNN_FV500",
            "PV_CNN_FV500",
            "LightweightFVCNN_FV500",
