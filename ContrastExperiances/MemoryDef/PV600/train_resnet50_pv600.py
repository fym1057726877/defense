...
class TrainClassifier:
    def __init__(
        ...
        model_name = "Resnet50_PV600"
        self.device = "cuda"
        self.classifier = Resnet50(num_classes=600).to(self.device)

        self.test_loader = testloader

        self.save_path = os.path.join(get_project_path("Defense"), "ContrastExperiances", "MemoryDef", "PV600", f"{model_name}.pth")
        self.classifier.load_state_dict(torch.load(self.save_path))

        self.train_loader = generateAdvData_for_memdef(classifier=self.classifier, attack_dataloader=trainloader,
                                                       eps=0.01)
