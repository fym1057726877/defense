...
from classifier.models import LightweightDeepConvNN
...
class TrainClassifier:
    def __init__(
        ...
        model_name = "LightweightFVCNN_PV600"
        self.device = "cuda"
        self.classifier = LightweightDeepConvNN(num_classes=600).to(self.device)

        self.train_loader, self.test_loader = trainloader, testloader

        self.save_path = os.path.join(get_project_path("Defense"), "classifier", "LightweightFVCNN",
                                      "PV600", f"{model_name}.pth")
