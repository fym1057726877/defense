...
class TrainClassifier:
    def __init__(
        ...
        self.device = "cuda"
        model_name = "Vit_PV220"
        self.classifier = VisionTransformer(img_size=64, patch_size=8, in_chans=1, num_classes=220,
                                            num_heads=4, mlp_ratio=2, embed_dim=512).to(self.device)

        self.train_loader, self.test_loader = trainloader, testloader

        self.save_path = os.path.join(get_project_path("Defense"), "classifier", "Vit", "PV220", f"{model_name}.pth")
    ...
    def train(self, epochs):
        ...
        for e in range(epochs):
            ...
            for _, (img, label) in tqdm(enumerate(self.train_loader), desc=f"train {e+1}/{epochs}", total=count):
