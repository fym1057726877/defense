...
class TrainClassifier:
    def __init__(
        ...
        self.device = "cuda"
        model_name = "Res2Net_PV600"
        self.classifier = create_model("res2net50_26w_4s", in_chans=1,
                                       num_classes=600, pretrained=False).to(self.device)

        self.train_loader, self.test_loader = trainloader, testloader

        self.save_path = os.path.join(get_project_path("Defense"), "classifier", "Res2net", "PV600", f"{model_name}.pth")
