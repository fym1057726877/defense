...
transform_train = transforms.Compose([
    ...
    transforms.ToTensor(),
    transforms.Normalize(mean=[0.5564], std=[0.0464])
])
...
class FV500(Dataset):
    def __init__(self, transform=None):
        super().__init__()
        path_dir_train = os.path.join(get_project_path("Defense"), "data", "FV500", "train")
        path_dir_test = os.path.join(get_project_path("Defense"), "data", "FV500", "test")
...
def get_CASIA_PV200_Dataloader(train=True, batch_size=16, shuffle=False, transform=None):
    ...
    dataset = FV500(transform=transform)
    num_classes = 500
...
trainloader = get_CASIA_PV200_Dataloader(train=True, batch_size=50, shuffle=True, transform=transform_train)
testloader = get_CASIA_PV200_Dataloader(train=False, batch_size=50, shuffle=False, transform=transform_test)
