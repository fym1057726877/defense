...
transform_train = transforms.Compose([
    ...
    transforms.ToTensor(),
    transforms.Normalize(mean=[0.443], std=[0.039])
])
...
class VERA_PV220(Dataset):
    def __init__(self, transform=None):
        super().__init__()
        path_dir_train = os.path.join(get_project_path(project_name="Defense"), "data", "VERA_PV220", "train")
        path_dir_val = os.path.join(get_project_path(project_name="Defense"), "data", "VERA_PV220", "val")
...
def get_VERA_PV220_Dataloader(train=True, batch_size=16, shuffle=False, transform=None):
    if transform is None:
        if train:
            transform = transform_train
        else:
            transform = transform_test
    dataset = VERA_PV220(transform=transform)
    num_classes = 220
    num_images_per_class = 10
    train_images_per_class = 8
    indices = []
...
trainloader = get_VERA_PV220_Dataloader(train=True, batch_size=40, shuffle=True, transform=transform_train)
testloader = get_VERA_PV220_Dataloader(train=False, batch_size=40, shuffle=False, transform=transform_test)
