import numpy as np
import torch as th
from ours.diffusion.models import GaussianDiffusion
...
def iter_denoise(unet_model, imgs, t=50, sampler="ddim", progress=False):
    assert t % 10 == 0
    diffusion = GaussianDiffusion()
    t_tensor = th.LongTensor([t] * imgs.shape[0]).to(imgs.device)
    x_t = diffusion.q_sample(x_start=imgs, t=t_tensor)
    if sampler == "ddim":
        ...
        spacediffusion = SpacedDiffusion(num_ddpm_timesteps=t + 10, num_ddim_timesteps=t + 10)
        final_sample = spacediffusion.p_sample_loop(unet_model, shape=imgs.shape, noise=x_t, progress=progress)
    else:
