...
def get_beta(betas_schedule="linear", num_timesteps=1000):
...
class GaussianDiffusion:
    def __init__(
        ...
        if betas is None:
            betas = get_beta(betas_schedule="linear", num_timesteps=num_timesteps)
        ...
        if mean_type == "eps":
            ...
            raise NotImplementedError(f"unknown mean_type {mean_type}")
    ...
    def p_mean_variance(self, model, x_t, t, clip_denoised=True):
        ...
        B, C = x_t.shape[:2]
        assert t.shape == (B,)
        model_output, mem_loss = model(x_t, t)
    ...
    def ddim_sample(
        ...
        """
        Sample x_{t-1} from the model using models.
    ...
    def ddim_sample_loop_progressive(
        ...
        """
        Use models to sample from the model and yield intermediate samples from
        each timestep of models.
    ...
    def training_losses(self, model, x_start, t, noise=None):
        ...
        loss = F.mse_loss(target, model_output) + mem_loss
        return loss

    def get_rand_t(self, batch_size, device, min_t=0, max_t=1000):
...
class UNetModel(nn.Module):
    ...
    :param use_checkpoint: use gradient checkpointing to reduce memory_v2 usage.
    ...
    def get_feature_vectors(self, x, timesteps, y=None):
        ...
        print("down:")
        for i in result["down"]:
            print(i.shape)

        print("middle:")
        for i in result["middle"]:
            print(i.shape)
...
class MemoryUnet(nn.Module):
    ...
    def __init__(
        ...
        self.memory = MemoryBlock(MEM_DIM=num_memories, features=features)
...
# ===========================MultilevelMemoryUnet=====================================
class MultilevelMemoryUnet(nn.Module):
    """
    The full UNet model with attention and timestep embedding.
    ...
    def __init__(
        ...
        self.memory1 = MemoryBlock(MEM_DIM=num_memories, features=features, threshold=threshold)
        self.memory2 = MemoryBlock(MEM_DIM=num_memories, features=features * 4, threshold=threshold)
        self.memory3 = MemoryBlock(MEM_DIM=num_memories, features=features * 4 * 4, threshold=threshold)
    ...
    def forward(self, x, timesteps, y=None):
        ...
        for module in self.input_blocks:
            ...
            hs.append(h)
        h = self.middle_block(h, emb)
        mem_loss1, mem_loss2, mem_loss3 = 0, 0, 0
        h, mem_loss1 = self.memory1(h)
        flag1, flag2 = True, True
        for module in self.output_blocks:
            cur_hs = hs.pop()
            if cur_hs.shape == (h.shape[0], 64, 8, 8) and flag1:
                cur_hs, mem_loss2 = self.memory2(cur_hs)
                flag1 = False
            if cur_hs.shape == (h.shape[0], 64, 16, 16) and flag2:
                cur_hs, mem_loss3 = self.memory3(cur_hs)
                flag2 = False

            cat_in = th.cat([h, cur_hs], dim=1)

            h = module(cat_in, emb)
        h = h.type(x.dtype)
        mem_loss = mem_loss1 + mem_loss2 + mem_loss3
        out = self.out(h)
        return out, mem_loss
...
class MemoryBlock(nn.Module):
    def __init__(self, MEM_DIM, features, sparse=True, threshold=1e-5, entropy_loss_coef=0.0002):
    ...
    def forward(self, x):
        ...
        z = x.view(B, -1)
        features = z.shape[1]
        assert self.features == features, f"self.features={self.features} but got input features={features}"
...
def test_unet():
    ...
    x = torch.rand(16, 4, 16, 16)
    t = torch.randint(0, 10, (16, ))
    # model.get_feature_vectors(x, t)
    out, loss = model(x, t)
