...
class Trainer:
    def __init__(
        ...
        self.model = MemoryAutoencoder(in_ch=1, ch=128, z_ch=4, out_ch=1, embed_dim=4, ch_mult=(1, 2, 4), resolution=64,
                                       num_res_blocks=2, attn_resolutions=(), dropout=0.0, double_z=False,
                                       num_memories=600, threshold=1e-4, recloss_type="l1", sparse=True,
                                       ).to(self.device)
        self.save_path = os.path.join(get_project_path(project_name="Defense"), "pretrained", "MemoryAE_LDM.pth")
